...
goal_id: goal-monitor-literature-risk-score-integration-001
title: Integrate literature-prior wallet and market risk scores into monitor review
status: active
...
- Literature and public surveillance-style case discussions motivate possible
  wallet and market risk features, but those features must be deterministic,
  explicitly marked as available or unavailable, and kept separate from Rule C.
- The proposed Whale-Agent formula is implemented as a Python
  literature-prior diagnostic score, not as an active agent, hard truth,
  misconduct finding, or trading signal.
deliverables:
...
- Add deterministic literature-prior wallet and market risk scores over the
  existing strict monitor candidates.
- Mark unavailable literature features, including wallet age, true new-wallet
  ratio, top-5 wallet concentration, and funding graph evidence, rather than
  guessing them.
- Surface the literature-prior scores in the human-review report and main
  dashboard as diagnostic context only.
...
- Existing compact strict-candidate review outputs and rolling monitor
  artifacts.
out_of_scope:
...
- Active Whale Agent implementation, Polygonscan/Dune enrichment, or new
  external API dependencies for the first literature-score integration.
acceptance_criteria:
...
- Literature-prior scores are generated by Python, use only existing local
  monitor artifacts, and do not replace Rule C.
- Literature-prior outputs include available versus unavailable feature status.
- Score thresholds are treated as diagnostic literature priors that require
  later calibration against Polymarket distributions and reference cases.
- Missing wallet-age, funding, true new-wallet-ratio, and per-wallet
  concentration data are marked `unavailable`, not zeroed as facts.
- Human-review and dashboard outputs expose score summaries without wallet
  addresses, order instructions, agents, MCP, ML, or live-trading paths.
...
next_commit: feat: add literature-prior monitor risk scores
...
- The first non-none strict monitor candidates now have a compact human-review
  report.
- Candidate review cards now explain the current high-priority AOC-2028
  candidate in terms of amount, concentration, and reference-label overlap.
...
- Literature-prior score outputs show which parts of the proposed paper-driven
  risk formulas are currently measurable and which require later Dune,
  Polygonscan, or per-wallet enrichment.

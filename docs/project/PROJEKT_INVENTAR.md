...
## Update 23.07.2026 (zweite Session): Trump-Post-Woche armiert und zwei gleichnamige Serien getrennt

**Neues Profil.** `trump_july20` (Event `715499`, «What will Trump post this week? (July 20 – July 26)»), Periode lückenlos an die Vorwoche anschliessend, Quelle und Matching unverändert zum bewährten Truth-Social-Pfad, Budget 400 USD. Von 17 Märkten waren beim Armieren 8 bereits zu 1.00 aufgelöst; 9 offen, davon 6 im mittleren Preisband (Israel 0.64, Wall Street 0.41, Soccer 0.335, President Xi 0.33, Scam 0.295, Pope 0.21). Tests 1011 grün, ruff sauber.

**Methodisch wichtiger Befund: zwei fast gleichnamige Serien schliessen einander aus.** Polymarket führt für dieselbe Person parallel «What will Trump **post** this week» (Serie 11341, Event 715499) und «What will Trump **say** this week» (Serie 11277, Event 723717). Die erste wertet ausschliesslich geschriebene Truth-Social-Beiträge, die zweite ausschliesslich Gesprochenes — ihr Regeltext schliesst geschriebene Nutzung wörtlich aus («Written usages of terms (e.g. Truth Social posts, written statements, etc.) will not count»). Ein Bot, der die eine Serie korrekt bedient, ist für die andere strukturell blind: Für die Say-Serie gäbe es keine einzelne Quelle, sondern über die Woche verstreute Reden, Pressetermine und Videoauftritte. Die Abgrenzung ist per Test im Code verankert (kein Trump-Profil darf auf die Say-Serie oder deren Slug zeigen). Konsequenz für die Auswertung: Marktbezeichnungen sind kein zuverlässiger Schlüssel, belastbar sind nur Event-ID und Regeltext.

**Neue Betriebsdokumentation.** `docs/project/BETRIEBSHANDBUCH_MENTIONS_BOTS.md` fasst erstmals zusammen, was bisher nur mündlich weitergegeben wurde: Unterschied zwischen dem einen echten Arbeitsordner und den temporären Arbeitskopien paralleler Sessions, der Weg jeder Änderung über Branch und Pull Request, ein Sechs-Schritte-Rezept für eine neue Wochen-Wette, die Verwechslungsgefahr ähnlich benannter Märkte sowie Betriebsbefehle und Notaus. Adressatin ist die Studentin.

**Betriebsstand.** `data/live/watchdog.json` wurde aufgeräumt: abgelaufene Wochen-Profile stehen auf inaktiv statt gelöscht (Historie bleibt nachvollziehbar), aktiv sind `trump_july20`, `elon_july20`, `mrbeast_gaming`, `lemonade_july22` und `hotones_july23`. Beim Pull kollidierten fünf generierte Artefakte aus `data/results/` (die tägliche Kette schreibt sie lokal und im Remote); sie wurden als `.lokal-2026-07-23.bak` gesichert und zurückgesetzt — die Kette erzeugt sie beim nächsten Lauf neu.

*(Dieser Abschnitt lag seit 23.07. nur uncommitted im Live-Klon und wurde am 28.07. beim Branch-Umzug aus `PROJEKT_INVENTAR.md.lokal-2026-07-28.bak` wieder eingegliedert.)*
...
## Update 28.07.2026 früh: Klon-Konvergenz, Michigan-Postmortem, drei Fixes, Graham armiert

**Klon-Konvergenz (löst den Koordinationshinweis oben auf).** Der Live-Klon ba-thesis läuft jetzt AUF `feat/earnings-bot` statt mit Handkopien: alle uncommitteten Kopien waren hash-identisch zum Branch (verifiziert, verlustfrei zurückgesetzt), der nie committete Inventar-Abschnitt vom 23.07. wurde aus dem .bak wieder eingegliedert (siehe oben). `origin/main` (PR #40, elon_july27/trump_july27) ist in den Branch gemergt — die Watchdog-Armierung der Rollover-Profile kann direkt auf dem Branch-Stand laufen; alles von main ist enthalten.

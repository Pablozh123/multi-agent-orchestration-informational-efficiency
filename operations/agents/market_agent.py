"""Deferred market-agent entry point.

The original implementation is preserved at:
    legacy/deferred_agents/market_agent.py

Market interpretation agents are deferred until bounded summary contracts,
...
def run_interpretation(*args: object, **kwargs: object) -> NoReturn:
    """Block accidental market-agent execution from the active code path."""
    raise RuntimeError(DEFERRED_MESSAGE)


def main() -> NoReturn:
    """Block command-style market-agent execution from the active code path."""
    raise RuntimeError(DEFERRED_MESSAGE)

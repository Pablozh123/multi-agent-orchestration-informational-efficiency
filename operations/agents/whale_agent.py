"""Deferred whale-agent entry point.

The original implementation is preserved at:
    legacy/deferred_agents/whale_agent.py

Wallet interpretation agents are deferred until bounded summary contracts,
...
def run_interpretation(*args: object, **kwargs: object) -> NoReturn:
    """Block accidental whale-agent execution from the active code path."""
    raise RuntimeError(DEFERRED_MESSAGE)


def main() -> NoReturn:
    """Block command-style whale-agent execution from the active code path."""
    raise RuntimeError(DEFERRED_MESSAGE)

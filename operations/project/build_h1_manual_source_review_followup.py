"""Build a focused H1 manual source-review follow-up worksheet."""
...
H1_FOLLOWUP_OUTPUT = "thesis_h1_manual_source_review_followup.csv"
H1_FOLLOWUP_DOC_OUTPUT = "THESIS_H1_MANUAL_SOURCE_REVIEW_FOLLOWUP.md"

H1_FOLLOWUP_COLUMNS: tuple[str, ...] = (
    "h1_followup_id",
    "review_order",
    "source_id",
    "source_title",
...
class H1ManualSourceReviewFollowupResult:
    """Generated H1 manual source-review follow-up paths and counts."""
...
def generate_h1_manual_source_review_followup(
    *,
    repo_root: Path = DEFAULT_REPO_ROOT,
    results_dir: Path = DEFAULT_RESULTS_DIR,
    docs_dir: Path = DEFAULT_DOCS_DIR,
) -> H1ManualSourceReviewFollowupResult:
    """Generate the H1 manual source-review follow-up CSV and Markdown."""
    ...
    followup = build_h1_manual_source_review_followup(execution=execution, ledger=ledger)
    _validate_followup(followup, repo_root=repo_root)

    results_dir.mkdir(parents=True, exist_ok=True)
    docs_dir.mkdir(parents=True, exist_ok=True)
    followup_path = results_dir / H1_FOLLOWUP_OUTPUT
    docs_path = docs_dir / H1_FOLLOWUP_DOC_OUTPUT
    followup.to_csv(followup_path, index=False)
    docs_path.write_text(_render_followup_doc(followup), encoding="utf-8")

    return H1ManualSourceReviewFollowupResult(
...
def build_h1_manual_source_review_followup(
    *,
    execution: pd.DataFrame,
    ledger: pd.DataFrame,
) -> pd.DataFrame:
    """Return the H1-only manual source-review starter list."""
    ...
    h1_rows = execution[execution["thesis_area"] == "H1"].sort_values("execution_order")
    rows: list[dict[str, object]] = []
    for review_order, row in enumerate(h1_rows.to_dict(orient="records"), start=1):
        ledger_id = str(row["ledger_id"])
        ledger_row = ledger_by_id.get(ledger_id)
        if ledger_row is None:
            raise ValueError(f"H1 follow-up missing ledger row for {ledger_id}.")
        page_note = _clean(ledger_row.get("page_or_section_note", ""))
        rows.append(
            {
                "h1_followup_id": f"h1_followup_{review_order:02d}_{row['source_id']}__{row['evidence_id']}",
                ...
                "guardrail_de": (
                    "H1 bleibt bounded: keine Quellenstatus-Hochstufung, keine "
                    "finale Zitation, keine Rohartefakt-Dumps, keine Runtime-Agenten; "
                    "spaetere Agentenhilfe nur mit separatem Goal, max 50 rows, "
                    "Tests und llm_audit_log."
                ),
            }
        )
    return pd.DataFrame(rows, columns=H1_FOLLOWUP_COLUMNS)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = generate_h1_manual_source_review_followup(
...
def _validate_followup(followup: pd.DataFrame, *, repo_root: Path) -> None:
    _require_columns(followup, H1_FOLLOWUP_COLUMNS, "H1 manual source-review follow-up")
    if len(followup) != 10:
        raise ValueError("H1 manual source-review follow-up must contain exactly 10 rows.")
    if followup["h1_followup_id"].duplicated().any():
        raise ValueError("H1 manual source-review follow-up contains duplicate IDs.")
    if followup["review_order"].astype(int).tolist() != list(range(1, 11)):
        raise ValueError("H1 manual source-review follow-up must be ordered 1..10.")
    if followup["source_status_change_allowed"].map(_bool_value).any():
        raise ValueError("H1 manual source-review follow-up must not allow source-status changes.")
    for column in H1_FOLLOWUP_COLUMNS:
        if followup[column].astype(str).str.len().eq(0).any():
            raise ValueError(f"H1 manual source-review follow-up contains empty {column}.")
    for artifact in followup["deterministic_artifact"].astype(str):
        if not (repo_root / artifact).exists():
            raise FileNotFoundError(f"H1 deterministic artifact missing: {artifact}")
    joined = "\n".join(followup.astype(str).agg(" ".join, axis=1).tolist())
    if chr(223) in joined:
        raise ValueError("H1 manual source-review follow-up must use Swiss spelling without sharp-s.")
    ...
    if missing:
        raise ValueError("H1 manual source-review follow-up missing required terms: " + ", ".join(missing))
...
def _render_followup_doc(followup: pd.DataFrame) -> str:
    ...
    return (
        ...
        "Dieses Artefakt startet den H1-Teil der manuellen Source Review. Es "
        "filtert die bestehende H1-H2-H3 Execution-Liste auf H1, verbindet sie "
        ...
        "und promotet keinen Quellenstatus.\n\n"
        "## Counts\n\n"
        f"- H1 follow-up rows: {len(followup)}\n"
        f"- Unique H1 sources: {int(followup['source_id'].nunique())}\n"
        ...
        + "\n\n"
        "## Use Rule\n\n"
        "Arbeite H1 manuell in dieser Reihenfolge ab. Fuer jede Zeile: Quelle "
        ...
        "belegt sind, bleiben H1-Zitationen final blockiert. Keine "
...
def _read_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Required H1 manual source-review follow-up input missing: {path}")

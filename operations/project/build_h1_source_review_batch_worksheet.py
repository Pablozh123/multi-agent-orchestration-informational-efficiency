"""Build a manual worksheet for the H1 source-review batch."""
...
WORKSHEET_OUTPUT = "thesis_h1_source_review_batch_worksheet.csv"
WORKSHEET_DOC_OUTPUT = "THESIS_H1_SOURCE_REVIEW_BATCH_WORKSHEET.md"
...
H1_BATCH_ID = "batch_plan_h1"
H1_EXECUTION_BATCH = "batch_01_h1_forecast_quality_source_review"


@dataclass(frozen=True)
class H1SourceReviewBatchWorksheetResult:
    """Generated H1 source-review worksheet paths and counts."""
...
def generate_h1_source_review_batch_worksheet(
    *,
    repo_root: Path = DEFAULT_REPO_ROOT,
    results_dir: Path = DEFAULT_RESULTS_DIR,
    docs_dir: Path = DEFAULT_DOCS_DIR,
) -> H1SourceReviewBatchWorksheetResult:
    """Generate the H1 source-review batch worksheet CSV and Markdown."""
    ...
    worksheet = build_h1_source_review_batch_worksheet(
    ...
    return H1SourceReviewBatchWorksheetResult(
...
def build_h1_source_review_batch_worksheet(
    *,
    execution_pass: pd.DataFrame,
    batch_plan: pd.DataFrame,
    update_checklist: pd.DataFrame,
) -> pd.DataFrame:
    """Return a 10-row H1 worksheet for manual source-review entry."""
    ...
    h1_rows = execution_pass.loc[execution_pass["thesis_area"] == "H1"].copy()
    h1_rows = h1_rows.sort_values(
    ...
    for order, row in enumerate(h1_rows.to_dict(orient="records"), start=1):
        source_id = _clean(row["source_id"])
        evidence_id = _clean(row["evidence_id"])
        item_type = _clean(row["item_type"])
        rows.append(
            {
                "worksheet_id": f"h1_source_review_{order:02d}_{source_id}__{evidence_id}",
                "worksheet_order": order,
                "thesis_area": "H1",
                "execution_batch": H1_EXECUTION_BATCH,
                "source_id": source_id,
                ...
                "blocked_wording_target_de": (
                    "Blocked-Wording pruefen: keine Reaktionsgeschwindigkeits-, "
                    "allgemeine Marktueberlegenheits-, RCP-Wahrscheinlichkeits- "
                    "oder Kausalclaims fuer H1."
                ),
                "citation_use_target_de": (
                    "Citation-Use erst nach Page-/Section-Note, Claim-Support "
                    "und Blocked-Wording setzen; pending bleibt sichtbar blockiert."
                ),
                "reviewer_metadata_target_de": (
                ...
                "blocked_actions_de": (
                    ...
                    "keine Runtime-Agenten, keine Rohartefakt-Dumps, max 50 rows "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = generate_h1_source_review_batch_worksheet(
...
def _validate_worksheet(
    *,
    ...
    _require_columns(worksheet, WORKSHEET_COLUMNS, "H1 source-review batch worksheet")
    _validate_inputs(
        execution_pass=execution_pass,
        batch_plan=batch_plan,
        update_checklist=update_checklist,
    ...
    if len(worksheet) != 10:
        raise ValueError("H1 source-review batch worksheet must contain 10 rows.")
    if worksheet["worksheet_order"].astype(int).tolist() != list(range(1, 11)):
        raise ValueError("H1 source-review batch worksheet must be ordered 1..10.")
    if worksheet["worksheet_id"].duplicated().any():
        raise ValueError("H1 source-review batch worksheet contains duplicate IDs.")
    if set(worksheet["thesis_area"].astype(str)) != {"H1"}:
        raise ValueError("H1 source-review batch worksheet must only contain H1 rows.")
    if set(worksheet["selected_table"].astype(str)) != {"T2"}:
        raise ValueError("H1 source-review batch worksheet must bind table T2.")
    if set(worksheet["selected_figure"].astype(str)) != {"F1"}:
        raise ValueError("H1 source-review batch worksheet must bind figure F1.")
    if int(worksheet["source_id"].nunique()) != 4:
        raise ValueError("H1 source-review batch worksheet expects 4 unique sources.")
    if int((worksheet["item_type"] == "method").sum()) != 4:
        raise ValueError("H1 source-review batch worksheet expects 4 method rows.")
    if int((worksheet["item_type"] == "interpretation").sum()) != 6:
        raise ValueError("H1 source-review batch worksheet expects 6 interpretation rows.")
    if int((worksheet["access_route"] == "external_locator_review").sum()) != 7:
        raise ValueError("H1 source-review batch worksheet expects 7 external locator rows.")
    if int((worksheet["access_route"] == "local_pdf_review").sum()) != 3:
        raise ValueError("H1 source-review batch worksheet expects 3 local PDF rows.")
    if not worksheet["ready_for_manual_entry"].map(_bool_value).all():
        raise ValueError("H1 source-review batch worksheet must be ready for manual entry.")
    if worksheet["ready_for_final_release"].map(_bool_value).any():
        raise ValueError("H1 source-review batch worksheet must not be final-release-ready.")
    if set(worksheet["current_review_status"].astype(str)) != {"pending_manual_review"}:
        raise ValueError("H1 source-review batch worksheet expects pending manual review rows.")
    if set(worksheet["current_citation_use_decision"].astype(str)) != {"blocked_pending_manual_review"}:
        raise ValueError("H1 source-review batch worksheet expects blocked pending citation rows.")
    joined = "\n".join(worksheet.fillna("").astype(str).agg(" ".join, axis=1).tolist())
    if chr(223) in joined:
        raise ValueError("H1 source-review batch worksheet must use Swiss spelling without sharp-s.")
    ...
    if missing:
        raise ValueError("H1 source-review batch worksheet missing required terms: " + ", ".join(missing))
...
def _validate_inputs(
    *,
    ...
    if not _bool_value(batch["ready_for_manual_execution"]):
        raise ValueError("H1 source-review batch worksheet requires manual-execution-ready H1 batch.")
    if _bool_value(batch["ready_for_final_release"]):
        raise ValueError("H1 source-review batch worksheet requires H1 batch not final-release-ready.")
    expected = {
        "source_review_rows": 10,
        "unique_sources": 4,
        "method_rows": 4,
        "interpretation_rows": 6,
        "external_locator_rows": 7,
        "local_pdf_rows": 3,
        "pending_citation_rows": 10,
        "final_ready_rows": 0,
    ...
    for column, expected_value in expected.items():
        if int(batch[column]) != expected_value:
            raise ValueError(f"H1 batch plan expected {column}={expected_value}.")
    if _clean(batch["selected_tables"]) != "T2" or _clean(batch["selected_figures"]) != "F1":
        raise ValueError("H1 source-review batch worksheet expects T2/F1 binding.")
    h1_rows = execution_pass.loc[execution_pass["thesis_area"] == "H1"]
    if len(h1_rows) != 10:
        raise ValueError("H1 source-review batch worksheet expects 10 H1 execution rows.")
    if h1_rows["source_status_change_allowed"].map(_bool_value).any():
        raise ValueError("H1 source-review batch worksheet must not allow source-status changes.")
    if h1_rows["final_citation_ready"].map(_bool_value).any():
        raise ValueError("H1 source-review batch worksheet expects 0 final-ready execution rows.")
    if len(update_checklist) != 8:
        raise ValueError("H1 source-review batch worksheet expects 8 update checklist rows.")
    if not update_checklist["ready_for_manual_update"].map(_bool_value).all():
        raise ValueError("H1 source-review batch worksheet requires manual-update-ready checklist rows.")
    if update_checklist["ready_for_final_citation_release"].map(_bool_value).any():
        raise ValueError("H1 source-review batch worksheet requires no final-release-ready checklist rows.")
...
def _render_worksheet_doc(worksheet: pd.DataFrame, batch_plan: pd.DataFrame) -> str:
    ...
    return (
        ...
        "Dieses Worksheet ist die manuelle Arbeitsliste fuer den ersten H1 "
        ...
        + "\n\n"
        "## Use Rule\n\n"
        "Arbeite H1 row-by-row: Quelle oeffnen, Page-/Section-Note eintragen, "
        "Claim-Support entscheiden, Blocked-Wording pruefen, Citation-Use "
        "setzen und reviewer metadata dokumentieren. Erlaubte Ledger-Felder "
        "bleiben `review_status`, `page_or_section_note`, "
        "`claim_support_decision`, `blocked_wording_check`, "
        "`citation_use_decision`, `reviewed_by`, `reviewed_at` und "
        "`review_comment_de`. Keine finale Zitation, keine "
        "Quellenstatus-Hochstufung, keine Rohartefakt-Dumps und keine "
...
def _batch_row(batch_plan: pd.DataFrame) -> pd.Series:
    rows = batch_plan.loc[batch_plan["batch_plan_id"] == H1_BATCH_ID]
    if len(rows) != 1:
        raise ValueError("H1 source-review batch worksheet requires one H1 batch-plan row.")
    return rows.iloc[0]


def _read_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Required H1 source-review worksheet input missing: {path}")

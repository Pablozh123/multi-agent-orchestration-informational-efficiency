"""Build a compact H1 source-review decision queue."""
...
QUEUE_OUTPUT = "thesis_h1_source_review_decision_queue.csv"
QUEUE_DOC_OUTPUT = "THESIS_H1_SOURCE_REVIEW_DECISION_QUEUE.md"
...
class H1SourceReviewDecisionQueueResult:
    """Generated H1 decision queue paths and counts."""
...
def generate_h1_source_review_decision_queue(
    *,
    repo_root: Path = DEFAULT_REPO_ROOT,
    results_dir: Path = DEFAULT_RESULTS_DIR,
    docs_dir: Path = DEFAULT_DOCS_DIR,
) -> H1SourceReviewDecisionQueueResult:
    """Generate the H1 source-review decision queue CSV and Markdown."""

    repo_root = Path(repo_root)
    results_dir = _resolve_under(repo_root, results_dir)
    docs_dir = _resolve_under(repo_root, docs_dir)

    h1_followup = _read_csv(results_dir / "thesis_h1_manual_source_review_followup.csv")
    ...
    queue = build_h1_source_review_decision_queue(
        h1_followup=h1_followup,
    ...
    return H1SourceReviewDecisionQueueResult(
...
def build_h1_source_review_decision_queue(
    *,
    h1_followup: pd.DataFrame,
    source_structure: pd.DataFrame,
    source_coverage: pd.DataFrame,
) -> pd.DataFrame:
    """Return a compact H1 source-review decision queue."""
    ...
    coverage_by_pair = {
        ...
        if str(row.get("thesis_area", "")) == "H1"
    }

    rows: list[dict[str, object]] = []
    for row in h1_followup.sort_values("review_order").to_dict(orient="records"):
        ...
        rows.append(
            {
                "decision_id": f"h1_decision_{int(row['review_order']):02d}_{source_id}__{evidence_id}",
                ...
                "required_manual_decision_fields_de": (
                    ...
                    "Citation-Use; Reviewer-Kommentar."
                ),
                ...
                "queue_status": "pending_manual_h1_source_review",
            }
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = generate_h1_source_review_decision_queue(
...
def _decision_focus(*, evidence_id: str, item_type: str) -> str:
    if item_type == "method":
        return "Methodenanker: Quelle gegen H1 Brier-/DM-Methodenwahl pruefen."
    if evidence_id == "interpretation_h1_bounded_advantage":
        return "Interpretationsgrenze: bounded H1 Support nur im definierten Vergleichsscope pruefen."
    if evidence_id == "interpretation_h1_broad_claim_not_proven":
        return "Interpretationsgrenze: breite Polymarket-Ueberlegenheitsbehauptung bleibt nicht belegt."
    return "H1 Entscheidungsgrenze manuell gegen Evidence ID pruefen."
...
def _manual_review_action(*, access_route: str, locator_task: str) -> str:
    if access_route == "local_pdf_review":
        return (
            ...
            "danach Claim-Support, Blocked-Wording und Citation-Use setzen. "
            f"Locator-Hinweis: {locator_task}"
        )
    return (
        "Externe DOI/JSTOR/URL manuell oeffnen; relevante Seite oder Abschnitt "
        "notieren; danach Claim-Support, Blocked-Wording und Citation-Use setzen. "
        f"Locator-Hinweis: {locator_task}"
    )
...
def _chapter_write_use(*, evidence_id: str, item_type: str) -> str:
    if item_type == "method":
        return (
            "Nach manueller Review als H1-Methodenanker verwenden; Ergebniswert "
            "bleibt ausschliesslich im deterministischen Artefakt."
        )
    if evidence_id == "interpretation_h1_bounded_advantage":
        return (
            "Nach manueller Review fuer bounded H1-Interpretation nutzen; "
            "keine allgemeine Ueberlegenheit formulieren."
        )
    if evidence_id == "interpretation_h1_broad_claim_not_proven":
        return (
            "Nach manueller Review fuer die Grenze der breiten H1-Behauptung "
            "nutzen; gemischte Evidenz sichtbar lassen."
        )
    return "Nach manueller Review nur im passenden H1-Abschnitt verwenden."


def _validate_queue(queue: pd.DataFrame, *, repo_root: Path) -> None:
    _require_columns(queue, QUEUE_COLUMNS, "H1 source-review decision queue")
    if queue["decision_id"].duplicated().any():
        raise ValueError("H1 decision queue contains duplicate decision_id values.")
    if queue["decision_order"].tolist() != list(range(1, len(queue) + 1)):
        raise ValueError("H1 decision queue order must be contiguous.")
    if len(queue) != 10:
        raise ValueError(f"H1 decision queue expected 10 rows, found {len(queue)}.")
    if queue["source_id"].nunique() != 4:
        raise ValueError("H1 decision queue must cover exactly four H1 sources.")
    if set(queue["item_type"]) != {"method", "interpretation"}:
        raise ValueError("H1 decision queue must cover method and interpretation rows.")
    if queue["selected_table"].nunique() != 1 or queue["selected_table"].iloc[0] != "T2":
        raise ValueError("H1 decision queue must stay bound to selected table T2.")
    if queue["selected_figure"].nunique() != 1 or queue["selected_figure"].iloc[0] != "F1":
        raise ValueError("H1 decision queue must stay bound to selected figure F1.")
    if queue["final_citation_ready"].map(_bool_value).any():
        raise ValueError("H1 decision queue must not mark rows final-citation-ready.")
    if queue["source_status_change_allowed"].map(_bool_value).any():
        raise ValueError("H1 decision queue must not allow source-status changes.")
    if not queue["primary_artifact_exists"].map(_bool_value).all():
        raise ValueError("H1 decision queue contains missing deterministic artifacts.")
    ...
    if missing_terms:
        raise ValueError("H1 decision queue missing required terms: " + ", ".join(missing_terms))
    if chr(223) in joined:
        raise ValueError("H1 decision queue contains German sharp-s.")
...
def _render_queue_doc(queue: pd.DataFrame) -> str:
    ...
    return (
        ...
        "Dieses Artefakt verdichtet die H1 Source Review auf eine konkrete "
        ...
        "## Counts\n\n"
        f"- H1 decision rows: {len(queue)}\n"
        f"- Unique H1 sources: {queue['source_id'].nunique()}\n"
        ...
        "Blocked-Wording, Citation-Use und Reviewer-Kommentar erfassen. Bis "
        "diese Felder belegt sind, bleibt H1 final blockiert: keine finale "
        ...
        "Zitation freigeben und keine Kennzahlen berechnen. Jede spaetere "
        "Nutzung braucht ein separates Goal, bounded inputs mit max 50 rows, "
        "Tests und `llm_audit_log`.\n"
    )
...
def _required_file(path: Path) -> None:
    if not path.exists():
        raise FileNotFoundError(f"Required H1 decision queue artifact missing: {path}")


def _read_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Required H1 decision queue input missing: {path}")

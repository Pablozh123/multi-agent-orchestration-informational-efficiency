"""Build a H1 guide for manually filling source-review ledger fields."""
...
GUIDE_OUTPUT = "thesis_h1_source_review_ledger_fill_guide.csv"
GUIDE_DOC_OUTPUT = "THESIS_H1_SOURCE_REVIEW_LEDGER_FILL_GUIDE.md"
...
class H1SourceReviewLedgerFillGuideResult:
    """Generated H1 ledger-fill guide paths and counts."""
...
def generate_h1_source_review_ledger_fill_guide(
    *,
    repo_root: Path = DEFAULT_REPO_ROOT,
    results_dir: Path = DEFAULT_RESULTS_DIR,
    docs_dir: Path = DEFAULT_DOCS_DIR,
) -> H1SourceReviewLedgerFillGuideResult:
    """Generate the H1 ledger-fill guide CSV and Markdown."""

    repo_root = Path(repo_root)
    results_dir = _resolve_under(repo_root, results_dir)
    docs_dir = _resolve_under(repo_root, docs_dir)

    worksheet = _read_csv(results_dir / "thesis_h1_source_review_batch_worksheet.csv")
    ...
    guide = build_h1_source_review_ledger_fill_guide(
        worksheet=worksheet,
        ledger=ledger,
    ...
    return H1SourceReviewLedgerFillGuideResult(
        guide_path=guide_path,
        docs_path=docs_path,
        guide_rows=len(guide),
        matched_ledger_rows=int((guide["ledger_match_status"] == "matched_h1_worksheet_to_ledger").sum()),
...
def build_h1_source_review_ledger_fill_guide(
    *,
    ...
    """Return a 10-row guide for filling H1 ledger rows manually."""

    _require_columns(worksheet, WORKSHEET_REQUIRED_COLUMNS, "H1 source-review worksheet")
    ...
    h1_worksheet = worksheet.loc[worksheet["thesis_area"].astype(str) == "H1"].copy()
    h1_ledger = ledger.loc[ledger["thesis_area"].astype(str) == "H1"].copy()
    ledger_by_key = {
        (str(row["source_id"]), str(row["evidence_id"])): row
        for row in h1_ledger.to_dict(orient="records")
    }
    rows: list[dict[str, object]] = []
    allowed_fields = "; ".join(ALLOWED_LEDGER_FIELDS)
    for order, worksheet_row in enumerate(
        h1_worksheet.sort_values("worksheet_order").to_dict(orient="records"),
        start=1,
    ):
        key = (str(worksheet_row["source_id"]), str(worksheet_row["evidence_id"]))
        ledger_row = ledger_by_key.get(key)
        if ledger_row is None:
            raise ValueError(f"H1 worksheet row has no matching ledger row: {key}.")
        rows.append(
            {
                "guide_id": f"h1_ledger_fill_{order:02d}_{key[0]}__{key[1]}",
                "guide_order": order,
                "thesis_area": "H1",
                "source_id": key[0],
                "evidence_id": key[1],
                ...
                "ledger_match_status": "matched_h1_worksheet_to_ledger",
                "allowed_ledger_fields": allowed_fields,
                "fill_sequence_de": (
                    "1 Page-/Section-Note erfassen; 2 Claim-Support entscheiden; "
                    "3 Blocked-Wording pruefen; 4 Citation-Use setzen; "
                    "5 reviewed_by, reviewed_at und review_comment_de dokumentieren; "
                    "6 Ledger regenerieren und preserved_manual_fields pruefen."
                ),
                ...
                "blocked_wording_instruction_de": str(
                    worksheet_row["blocked_wording_target_de"]
                ),
                ...
                "blocked_actions_de": (
                    ...
                    "keine Quellenstatus-Hochstufung, keine Rohartefakt-Dumps, keine "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = generate_h1_source_review_ledger_fill_guide(
...
def _validate_guide(
    *,
    ...
    _require_columns(guide, GUIDE_COLUMNS, "H1 source-review ledger fill guide")
    _validate_inputs(
        worksheet=worksheet,
        ledger=ledger,
        update_checklist=update_checklist,
    ...
    if len(guide) != 10:
        raise ValueError("H1 ledger-fill guide must contain 10 rows.")
    if guide["guide_order"].astype(int).tolist() != list(range(1, 11)):
        raise ValueError("H1 ledger-fill guide must be ordered 1..10.")
    if guide["guide_id"].duplicated().any():
        raise ValueError("H1 ledger-fill guide contains duplicate IDs.")
    if set(guide["thesis_area"].astype(str)) != {"H1"}:
        raise ValueError("H1 ledger-fill guide must only contain H1 rows.")
    if set(guide["ledger_match_status"].astype(str)) != {"matched_h1_worksheet_to_ledger"}:
        raise ValueError("H1 ledger-fill guide requires all rows matched to the ledger.")
    if int(guide["source_id"].nunique()) != 4:
        raise ValueError("H1 ledger-fill guide expects 4 unique sources.")
    if int((guide["item_type"] == "method").sum()) != 4:
        raise ValueError("H1 ledger-fill guide expects 4 method rows.")
    if int((guide["item_type"] == "interpretation").sum()) != 6:
        raise ValueError("H1 ledger-fill guide expects 6 interpretation rows.")
    if int((guide["access_route"] == "external_locator_review").sum()) != 7:
        raise ValueError("H1 ledger-fill guide expects 7 external locator rows.")
    if int((guide["access_route"] == "local_pdf_review").sum()) != 3:
        raise ValueError("H1 ledger-fill guide expects 3 local PDF rows.")
    if set(guide["selected_table"].astype(str)) != {"T2"}:
        raise ValueError("H1 ledger-fill guide must bind table T2.")
    if set(guide["selected_figure"].astype(str)) != {"F1"}:
        raise ValueError("H1 ledger-fill guide must bind figure F1.")
    if not guide["ready_for_manual_ledger_entry"].map(_bool_value).all():
        raise ValueError("H1 ledger-fill guide must be ready for manual ledger entry.")
    if guide["ready_for_final_release"].map(_bool_value).any():
        raise ValueError("H1 ledger-fill guide must not be final-release-ready.")
    if set(guide["current_ledger_progress_state"].astype(str)) != {"pending_manual_review"}:
        raise ValueError("H1 ledger-fill guide expects pending ledger progress.")
    if set(guide["current_ledger_citation_use_decision"].astype(str)) != {
        "blocked_pending_manual_review"
    }:
        raise ValueError("H1 ledger-fill guide expects blocked pending citation rows.")
    for artifact in guide["deterministic_artifact"].astype(str):
        if artifact and artifact.lower() != "nan" and not (repo_root / artifact).exists():
            raise FileNotFoundError(f"H1 ledger-fill guide artifact missing: {artifact}")
    joined = "\n".join(guide.fillna("").astype(str).agg(" ".join, axis=1).tolist())
    if chr(223) in joined:
        raise ValueError("H1 ledger-fill guide must use Swiss spelling without sharp-s.")
    ...
    if missing:
        raise ValueError("H1 ledger-fill guide missing required terms: " + ", ".join(missing))
...
def _validate_inputs(
    *,
    ...
    h1_worksheet = worksheet.loc[worksheet["thesis_area"].astype(str) == "H1"]
    h1_ledger = ledger.loc[ledger["thesis_area"].astype(str) == "H1"]
    if len(h1_worksheet) != 10:
        raise ValueError("H1 ledger-fill guide expects 10 H1 worksheet rows.")
    if len(h1_ledger) != 10:
        raise ValueError("H1 ledger-fill guide expects 10 H1 ledger rows.")
    worksheet_keys = _keys(h1_worksheet)
    ledger_keys = _keys(h1_ledger)
    if worksheet_keys != ledger_keys:
        raise ValueError("H1 worksheet and ledger rows must match by source_id and evidence_id.")
    if h1_worksheet["ready_for_final_release"].map(_bool_value).any():
        raise ValueError("H1 worksheet must not be final-release-ready.")
    if h1_ledger["source_status_change_allowed"].map(_bool_value).any():
        raise ValueError("H1 ledger-fill guide must not allow source-status changes.")
    if h1_ledger["final_citation_ready"].map(_bool_value).any():
        raise ValueError("H1 ledger-fill guide expects 0 final-ready ledger rows.")
    h1_alignment = alignment.loc[alignment["slice_id"].astype(str) == "H1"]
    if len(h1_alignment) != 1:
        raise ValueError("H1 ledger-fill guide requires one H1 alignment row.")
    row = h1_alignment.iloc[0]
    if int(row["ledger_rows"]) != 10 or int(row["matched_rows"]) != 10:
        raise ValueError("H1 ledger-fill guide requires 10 matched H1 ledger rows.")
    for column in ("queue_missing_ledger_rows", "ledger_missing_queue_rows", "field_mismatch_rows"):
        if int(row[column]) != 0:
            raise ValueError("H1 ledger-fill guide requires 0 alignment gaps.")
    if int(row["ledger_final_ready_rows"]) != 0:
        raise ValueError("H1 ledger-fill guide expects 0 final-ready H1 ledger rows.")
    if int(row["ledger_source_status_change_rows"]) != 0:
        raise ValueError("H1 ledger-fill guide expects 0 H1 source-status change rows.")
    if str(row["selected_tables"]) != "T2" or str(row["selected_figures"]) != "F1":
        raise ValueError("H1 ledger-fill guide expects T2/F1 alignment.")
    if len(update_checklist) != 8:
        raise ValueError("H1 ledger-fill guide expects 8 manual update checklist rows.")
    if not update_checklist["ready_for_manual_update"].map(_bool_value).all():
        raise ValueError("H1 ledger-fill guide requires manual-update-ready checklist rows.")
    if update_checklist["ready_for_final_citation_release"].map(_bool_value).any():
        raise ValueError("H1 ledger-fill guide requires no final-release-ready checklist rows.")
...
def _render_guide_doc(guide: pd.DataFrame) -> str:
    ...
    return (
        ...
        "Dieser Guide zeigt, wie die zehn H1 Worksheet-Zeilen manuell in den "
        ...
        "und erzeugt keine finale Zitation.\n\n"
        "## Counts\n\n"
        f"- Guide rows: {len(guide)}\n"
        f"- Matched ledger rows: {int((guide['ledger_match_status'] == 'matched_h1_worksheet_to_ledger').sum())}\n"
        ...
        "`reviewed_at` und `review_comment_de`. Danach den Ledger regenerieren "
        "und pruefen, ob `preserved_manual_fields=True` und der "
        "`review_progress_state` plausibel gesetzt ist. Keine finale Zitation, "
        "keine Quellenstatus-Hochstufung, keine Rohartefakt-Dumps, keine "
        "Runtime-Agenten, kein MCP, kein Model Routing, keine LLM-Metriken, "
        "max 50 rows und `llm_audit_log` fuer spaetere Agentenhilfe.\n"
    )
...
def _read_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Required H1 ledger-fill guide input missing: {path}")

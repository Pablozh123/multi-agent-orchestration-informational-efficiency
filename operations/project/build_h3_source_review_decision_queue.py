"""Build a compact H3 source-review decision queue."""
...
QUEUE_OUTPUT = "thesis_h3_source_review_decision_queue.csv"
QUEUE_DOC_OUTPUT = "THESIS_H3_SOURCE_REVIEW_DECISION_QUEUE.md"
...
QUEUE_COLUMNS: tuple[str, ...] = (
    ...
    "h3_granger_boundary_de",
    "h3_wallet_boundary_de",
...
class H3SourceReviewDecisionQueueResult:
    """Generated H3 decision queue paths and counts."""
...
def generate_h3_source_review_decision_queue(
    *,
    repo_root: Path = DEFAULT_REPO_ROOT,
    results_dir: Path = DEFAULT_RESULTS_DIR,
    docs_dir: Path = DEFAULT_DOCS_DIR,
) -> H3SourceReviewDecisionQueueResult:
    """Generate the H3 source-review decision queue CSV and Markdown."""

    repo_root = Path(repo_root)
    results_dir = _resolve_under(repo_root, results_dir)
    docs_dir = _resolve_under(repo_root, docs_dir)

    h3_followup = _read_csv(results_dir / "thesis_h3_manual_source_review_followup.csv")
    ...
    queue = build_h3_source_review_decision_queue(
        h3_followup=h3_followup,
    ...
    return H3SourceReviewDecisionQueueResult(
...
def build_h3_source_review_decision_queue(
    *,
    h3_followup: pd.DataFrame,
    source_structure: pd.DataFrame,
    source_coverage: pd.DataFrame,
) -> pd.DataFrame:
    """Return a compact H3 source-review decision queue."""
    ...
    _require_columns(
        ...
        "H3 manual source review follow-up",
    )
    _require_columns(
        source_structure,
        (
    ...
    coverage_by_pair = {
        ...
        if str(row.get("thesis_area", "")) == "H3"
    }

    rows: list[dict[str, object]] = []
    for row in h3_followup.sort_values("review_order").to_dict(orient="records"):
        ...
        rows.append(
            {
                "decision_id": f"h3_decision_{int(row['review_order']):02d}_{source_id}__{evidence_id}",
                ...
                "required_manual_decision_fields_de": (
                    ...
                    "Citation-Use; Granger-Grenze; Wallet-Grenze; Reviewer-Kommentar."
                ),
                ...
                "h3_granger_boundary_de": (
                    "H3 darf Granger nur als predictive timing diagnostic unter "
                    "Modellannahmen formulieren: keine Kausalclaims, keine "
                    "Private-Information-Beweise, keine Profitabilitaetsclaims; "
                    "taegliche Ausrichtung, Mehrfachtests und BUY-only-Extraktion "
                    "begrenzen die Schlussstaerke."
                ),
                "h3_wallet_boundary_de": (
                    "H3 Wallet-Tiers bleiben dataset-relativ: keine willkuerlichen "
                    "Whale-Schwellen, keine Wallet-Adressen, keine Identifikation "
                    "privater Information und keine Trading-Claims."
                ),
                ...
                "agent_assist_boundary_de": (
                    ...
                    "keine Quelleninhalte bewerten, keine Seitenzahlen erfinden, "
                    "keine Granger- oder Wallet-Grenzen lockern, keine Zitation "
                    "freigeben, keine Kennzahlen berechnen, max 50 rows und "
                    "llm_audit_log."
                ),
                "source_status_change_allowed": False,
                "final_citation_ready": False,
                "queue_status": "pending_manual_h3_source_review",
            }
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = generate_h3_source_review_decision_queue(
...
def _decision_focus(*, evidence_id: str, item_type: str) -> str:
    if evidence_id == "method_h3_granger_timing":
        return "Methodenanker: Quelle gegen Granger-/Lead-Lag-Diagnostik pruefen; keine Kausalitaet ableiten."
    if evidence_id == "method_h3_wallet_tiers":
        return "Methodenanker: Quelle gegen dataset-relative Wallet-Tiers pruefen; keine festen Whale-Schwellen."
    if item_type == "interpretation":
        return "Interpretationsgrenze: top-tier timing pattern als bounded predictive diagnostic pruefen."
    return "H3 Entscheidungsgrenze manuell gegen Evidence ID pruefen."
...
def _manual_review_action(*, access_route: str, locator_task: str) -> str:
    if access_route == "local_pdf_review":
        return (
            ...
            "danach Claim-Support, Blocked-Wording, Citation-Use, Granger-Grenze "
            f"und Wallet-Grenze setzen. Locator-Hinweis: {locator_task}"
        )
    return (
        "Externe DOI/JSTOR/URL manuell oeffnen; relevante Seite oder Abschnitt "
        "notieren; danach Claim-Support, Blocked-Wording, Citation-Use, "
        f"Granger-Grenze und Wallet-Grenze setzen. Locator-Hinweis: {locator_task}"
    )
...
def _chapter_write_use(*, evidence_id: str, item_type: str) -> str:
    if evidence_id == "method_h3_granger_timing":
        return (
            "Nach manueller Review als H3-Methodenanker fuer Lead-Lag- und "
            "Granger-Diagnostik verwenden; Kennzahlen bleiben im deterministischen "
            "Artefakt."
        )
    if evidence_id == "method_h3_wallet_tiers":
        return (
            "Nach manueller Review als H3-Methodenanker fuer dataset-relative "
            "Wallet-Tiers verwenden; keine Wallet-Adressen oder festen Whale-"
            "Schwellen formulieren."
        )
    if item_type == "interpretation":
        return (
            "Nach manueller Review fuer das bounded top-tier timing pattern nutzen; "
            "keine Private-Information-, Profitabilitaets- oder Trading-Claims."
        )
    return "Nach manueller Review nur im passenden H3-Abschnitt verwenden."


def _validate_queue(queue: pd.DataFrame, *, repo_root: Path) -> None:
    _require_columns(queue, QUEUE_COLUMNS, "H3 source-review decision queue")
    if queue["decision_id"].duplicated().any():
        raise ValueError("H3 decision queue contains duplicate decision_id values.")
    if queue["decision_order"].tolist() != list(range(1, len(queue) + 1)):
        raise ValueError("H3 decision queue order must be contiguous.")
    if len(queue) != 8:
        raise ValueError(f"H3 decision queue expected 8 rows, found {len(queue)}.")
    if queue["source_id"].nunique() != 4:
        raise ValueError("H3 decision queue must cover exactly four H3 sources.")
    if set(queue["item_type"]) != {"method", "interpretation"}:
        raise ValueError("H3 decision queue must cover method and interpretation rows.")
    if queue["selected_table"].nunique() != 1 or queue["selected_table"].iloc[0] != "T4":
        raise ValueError("H3 decision queue must stay bound to selected table T4.")
    if queue["selected_figure"].nunique() != 1 or queue["selected_figure"].iloc[0] != "F3":
        raise ValueError("H3 decision queue must stay bound to selected figure F3.")
    if int((queue["item_type"] == "method").sum()) != 5:
        raise ValueError("H3 decision queue expected 5 method rows.")
    if int((queue["item_type"] == "interpretation").sum()) != 3:
        raise ValueError("H3 decision queue expected 3 interpretation rows.")
    if queue["final_citation_ready"].map(_bool_value).any():
        raise ValueError("H3 decision queue must not mark rows final-citation-ready.")
    if queue["source_status_change_allowed"].map(_bool_value).any():
        raise ValueError("H3 decision queue must not allow source-status changes.")
    if not queue["primary_artifact_exists"].map(_bool_value).all():
        raise ValueError("H3 decision queue contains missing deterministic artifacts.")
    ...
    if missing_terms:
        raise ValueError("H3 decision queue missing required terms: " + ", ".join(missing_terms))
    if chr(223) in joined:
        raise ValueError("H3 decision queue contains German sharp-s.")
...
def _render_queue_doc(queue: pd.DataFrame) -> str:
    ...
    return (
        ...
        "Dieses Artefakt verdichtet die H3 Source Review auf eine konkrete "
        ...
        "## Counts\n\n"
        f"- H3 decision rows: {len(queue)}\n"
        f"- Unique H3 sources: {queue['source_id'].nunique()}\n"
        ...
        "Blocked-Wording, Citation-Use, Granger-Grenze, Wallet-Grenze und "
        "Reviewer-Kommentar erfassen. Bis diese Felder belegt sind, bleibt H3 "
        ...
        "## H3 Boundary\n\n"
        "H3 darf Granger- und Lead-Lag-Signale nur als predictive timing "
        "diagnostics unter Modellannahmen formulieren. Die Queue blockiert "
        "Kausalclaims, Private-Information-Beweise, Profitabilitaetsclaims, "
        "Trading-Claims, willkuerliche Whale-Schwellen und Wallet-Adressen.\n\n"
        ...
        "Granger-Grenze oder Wallet-Grenze lockern, keine Zitation freigeben "
        "und keine Kennzahlen berechnen. Jede spaetere Nutzung braucht ein "
        "separates Goal, bounded inputs mit max 50 rows, Tests und "
        "`llm_audit_log`.\n"
    )
...
def _required_file(path: Path) -> None:
    if not path.exists():
        raise FileNotFoundError(f"Required H3 decision queue artifact missing: {path}")


def _read_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Required H3 decision queue input missing: {path}")

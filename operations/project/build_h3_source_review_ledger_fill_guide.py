"""Build a H3 guide for manually filling source-review ledger fields."""
...
GUIDE_OUTPUT = "thesis_h3_source_review_ledger_fill_guide.csv"
GUIDE_DOC_OUTPUT = "THESIS_H3_SOURCE_REVIEW_LEDGER_FILL_GUIDE.md"
...
class H3SourceReviewLedgerFillGuideResult:
    """Generated H3 ledger-fill guide paths and counts."""
...
def generate_h3_source_review_ledger_fill_guide(
    *,
    repo_root: Path = DEFAULT_REPO_ROOT,
    results_dir: Path = DEFAULT_RESULTS_DIR,
    docs_dir: Path = DEFAULT_DOCS_DIR,
) -> H3SourceReviewLedgerFillGuideResult:
    """Generate the H3 ledger-fill guide CSV and Markdown."""

    repo_root = Path(repo_root)
    results_dir = _resolve_under(repo_root, results_dir)
    docs_dir = _resolve_under(repo_root, docs_dir)

    worksheet = _read_csv(results_dir / "thesis_h3_source_review_batch_worksheet.csv")
    ...
    guide = build_h3_source_review_ledger_fill_guide(
        worksheet=worksheet,
        ledger=ledger,
    ...
    return H3SourceReviewLedgerFillGuideResult(
        guide_path=guide_path,
        docs_path=docs_path,
        guide_rows=len(guide),
        matched_ledger_rows=int((guide["ledger_match_status"] == "matched_h3_worksheet_to_ledger").sum()),
...
def build_h3_source_review_ledger_fill_guide(
    *,
    ...
    """Return an 8-row guide for filling H3 ledger rows manually."""

    _require_columns(worksheet, WORKSHEET_REQUIRED_COLUMNS, "H3 source-review worksheet")
    ...
    h3_worksheet = worksheet.loc[worksheet["thesis_area"].astype(str) == "H3"].copy()
    h3_ledger = ledger.loc[ledger["thesis_area"].astype(str) == "H3"].copy()
    ledger_by_key = {
        (str(row["source_id"]), str(row["evidence_id"])): row
        for row in h3_ledger.to_dict(orient="records")
    }
    rows: list[dict[str, object]] = []
    allowed_fields = "; ".join(ALLOWED_LEDGER_FIELDS)
    for order, worksheet_row in enumerate(
        h3_worksheet.sort_values("worksheet_order").to_dict(orient="records"),
        start=1,
    ):
        key = (str(worksheet_row["source_id"]), str(worksheet_row["evidence_id"]))
        ledger_row = ledger_by_key.get(key)
        if ledger_row is None:
            raise ValueError(f"H3 worksheet row has no matching ledger row: {key}.")
        rows.append(
            {
                "guide_id": f"h3_ledger_fill_{order:02d}_{key[0]}__{key[1]}",
                "guide_order": order,
                "thesis_area": "H3",
                "source_id": key[0],
                "evidence_id": key[1],
                ...
                "ledger_match_status": "matched_h3_worksheet_to_ledger",
                "allowed_ledger_fields": allowed_fields,
                "fill_sequence_de": (
                    "1 Page-/Section-Note erfassen; 2 Claim-Support entscheiden; "
                    "3 Blocked-Wording pruefen; 4 H3 Granger-Grenze pruefen; "
                    "5 H3 Wallet-Grenze pruefen; 6 Citation-Use setzen; "
                    "7 reviewed_by, reviewed_at und review_comment_de dokumentieren; "
                    "8 Ledger regenerieren und preserved_manual_fields pruefen."
                ),
                ...
                "blocked_wording_instruction_de": (
                    ...
                    + " H3-spezifisch: "
                    + str(worksheet_row["granger_boundary_de"])
                    + " "
                    + str(worksheet_row["wallet_boundary_de"])
                ),
                ...
                "blocked_actions_de": (
                    ...
                    "Profitabilitaetsclaims, keine Rohartefakt-Dumps, keine Runtime-Agenten, "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = generate_h3_source_review_ledger_fill_guide(
...
def _validate_guide(
    *,
    ...
    _require_columns(guide, GUIDE_COLUMNS, "H3 source-review ledger fill guide")
    _validate_inputs(
        worksheet=worksheet,
        ledger=ledger,
        update_checklist=update_checklist,
    ...
    if len(guide) != 8:
        raise ValueError("H3 ledger-fill guide must contain 8 rows.")
    if guide["guide_order"].astype(int).tolist() != list(range(1, 9)):
        raise ValueError("H3 ledger-fill guide must be ordered 1..8.")
    if guide["guide_id"].duplicated().any():
        raise ValueError("H3 ledger-fill guide contains duplicate IDs.")
    if set(guide["thesis_area"].astype(str)) != {"H3"}:
        raise ValueError("H3 ledger-fill guide must only contain H3 rows.")
    if set(guide["ledger_match_status"].astype(str)) != {"matched_h3_worksheet_to_ledger"}:
        raise ValueError("H3 ledger-fill guide requires all rows matched to the ledger.")
    if int(guide["source_id"].nunique()) != 4:
        raise ValueError("H3 ledger-fill guide expects 4 unique sources.")
    if int((guide["item_type"] == "method").sum()) != 5:
        raise ValueError("H3 ledger-fill guide expects 5 method rows.")
    if int((guide["item_type"] == "interpretation").sum()) != 3:
        raise ValueError("H3 ledger-fill guide expects 3 interpretation rows.")
    if int((guide["access_route"] == "external_locator_review").sum()) != 2:
        raise ValueError("H3 ledger-fill guide expects 2 external locator rows.")
    if int((guide["access_route"] == "local_pdf_review").sum()) != 6:
        raise ValueError("H3 ledger-fill guide expects 6 local PDF rows.")
    if set(guide["selected_table"].astype(str)) != {"T4"}:
        raise ValueError("H3 ledger-fill guide must bind table T4.")
    if set(guide["selected_figure"].astype(str)) != {"F3"}:
        raise ValueError("H3 ledger-fill guide must bind figure F3.")
    if not guide["ready_for_manual_ledger_entry"].map(_bool_value).all():
        raise ValueError("H3 ledger-fill guide must be ready for manual ledger entry.")
    if guide["ready_for_final_release"].map(_bool_value).any():
        raise ValueError("H3 ledger-fill guide must not be final-release-ready.")
    if set(guide["current_ledger_progress_state"].astype(str)) != {"pending_manual_review"}:
        raise ValueError("H3 ledger-fill guide expects pending ledger progress.")
    if set(guide["current_ledger_citation_use_decision"].astype(str)) != {
        "blocked_pending_manual_review"
    }:
        raise ValueError("H3 ledger-fill guide expects blocked pending citation rows.")
    for artifact in guide["deterministic_artifact"].astype(str):
        if artifact and artifact.lower() != "nan" and not (repo_root / artifact).exists():
            raise FileNotFoundError(f"H3 ledger-fill guide artifact missing: {artifact}")
    joined = "\n".join(guide.fillna("").astype(str).agg(" ".join, axis=1).tolist())
    if chr(223) in joined:
        raise ValueError("H3 ledger-fill guide must use Swiss spelling without sharp-s.")
    ...
    if missing:
        raise ValueError("H3 ledger-fill guide missing required terms: " + ", ".join(missing))
...
def _validate_inputs(
    *,
    ...
    h3_worksheet = worksheet.loc[worksheet["thesis_area"].astype(str) == "H3"]
    h3_ledger = ledger.loc[ledger["thesis_area"].astype(str) == "H3"]
    if len(h3_worksheet) != 8:
        raise ValueError("H3 ledger-fill guide expects 8 H3 worksheet rows.")
    if len(h3_ledger) != 8:
        raise ValueError("H3 ledger-fill guide expects 8 H3 ledger rows.")
    worksheet_keys = _keys(h3_worksheet)
    ledger_keys = _keys(h3_ledger)
    if worksheet_keys != ledger_keys:
        raise ValueError("H3 worksheet and ledger rows must match by source_id and evidence_id.")
    if h3_worksheet["ready_for_final_release"].map(_bool_value).any():
        raise ValueError("H3 worksheet must not be final-release-ready.")
    if h3_ledger["source_status_change_allowed"].map(_bool_value).any():
        raise ValueError("H3 ledger-fill guide must not allow source-status changes.")
    if h3_ledger["final_citation_ready"].map(_bool_value).any():
        raise ValueError("H3 ledger-fill guide expects 0 final-ready ledger rows.")
    h3_alignment = alignment.loc[alignment["slice_id"].astype(str) == "H3"]
    if len(h3_alignment) != 1:
        raise ValueError("H3 ledger-fill guide requires one H3 alignment row.")
    row = h3_alignment.iloc[0]
    if int(row["ledger_rows"]) != 8 or int(row["matched_rows"]) != 8:
        raise ValueError("H3 ledger-fill guide requires 8 matched H3 ledger rows.")
    for column in ("queue_missing_ledger_rows", "ledger_missing_queue_rows", "field_mismatch_rows"):
        if int(row[column]) != 0:
            raise ValueError("H3 ledger-fill guide requires 0 alignment gaps.")
    if int(row["ledger_final_ready_rows"]) != 0:
        raise ValueError("H3 ledger-fill guide expects 0 final-ready H3 ledger rows.")
    if int(row["ledger_source_status_change_rows"]) != 0:
        raise ValueError("H3 ledger-fill guide expects 0 H3 source-status change rows.")
    if str(row["selected_tables"]) != "T4" or str(row["selected_figures"]) != "F3":
        raise ValueError("H3 ledger-fill guide expects T4/F3 alignment.")
    if len(update_checklist) != 8:
        raise ValueError("H3 ledger-fill guide expects 8 manual update checklist rows.")
    if not update_checklist["ready_for_manual_update"].map(_bool_value).all():
        raise ValueError("H3 ledger-fill guide requires manual-update-ready checklist rows.")
    if update_checklist["ready_for_final_citation_release"].map(_bool_value).any():
        raise ValueError("H3 ledger-fill guide requires no final-release-ready checklist rows.")
...
def _render_guide_doc(guide: pd.DataFrame) -> str:
    ...
    return (
        ...
        "Dieser Guide zeigt, wie die acht H3 Worksheet-Zeilen manuell in den "
        ...
        "und erzeugt keine finale Zitation. Die H3 Granger-Grenze und "
        "Wallet-Grenze bleiben pro Zeile sichtbar.\n\n"
        "## Counts\n\n"
        f"- Guide rows: {len(guide)}\n"
        f"- Matched ledger rows: {int((guide['ledger_match_status'] == 'matched_h3_worksheet_to_ledger').sum())}\n"
        ...
        "`reviewed_at` und `review_comment_de`. Fuer H3 zusaetzlich "
        "Granger-Grenze und Wallet-Grenze pruefen: keine Kausalclaims, keine "
        "Private-Information-Beweise, keine willkuerlichen Whale-Schwellen, "
        "keine Wallet-Adressen, keine Trading-Claims und keine "
        "Profitabilitaetsclaims. Danach den Ledger regenerieren und pruefen, "
        "ob `preserved_manual_fields=True` und der `review_progress_state` "
        "plausibel gesetzt ist. Keine finale Zitation, keine "
        "Quellenstatus-Hochstufung, keine Rohartefakt-Dumps, keine "
...
def _read_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Required H3 ledger-fill guide input missing: {path}")

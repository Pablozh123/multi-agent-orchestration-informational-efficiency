...
from operations.project.build_h1_source_review_batch_worksheet import (
    WORKSHEET_COLUMNS,
    generate_h1_source_review_batch_worksheet,
)


def test_generate_h1_source_review_batch_worksheet_writes_ten_rows(tmp_path: Path) -> None:
    _write_fixture(tmp_path)

    result = generate_h1_source_review_batch_worksheet(repo_root=tmp_path)

    worksheet = pd.read_csv(result.worksheet_path)
    doc = result.docs_path.read_text(encoding="utf-8")

    assert tuple(worksheet.columns) == WORKSHEET_COLUMNS
    assert result.worksheet_rows == 10
    assert result.unique_sources == 4
    assert result.method_rows == 4
    assert result.interpretation_rows == 6
    assert result.pending_citation_rows == 10
    assert result.final_release_ready_rows == 0
    assert worksheet["worksheet_order"].tolist() == list(range(1, 11))
    assert set(worksheet["thesis_area"]) == {"H1"}
    assert set(worksheet["selected_table"]) == {"T2"}
    assert set(worksheet["selected_figure"]) == {"F1"}
    ...
    assert "H1 Source Review Batch Worksheet" in doc
    assert "Worksheet rows: 10" in doc
    assert "Final release ready rows: 0" in doc
    assert chr(223) not in doc


def test_h1_source_review_batch_worksheet_keeps_manual_fields_and_guards_visible(
    tmp_path: Path,
) -> None:
    _write_fixture(tmp_path)
...
def test_h1_source_review_batch_worksheet_rejects_final_ready_execution_row(
    tmp_path: Path,
) -> None:
    _write_fixture(tmp_path)
    ...
    with pytest.raises(ValueError, match="0 final-ready execution rows"):
        generate_h1_source_review_batch_worksheet(repo_root=tmp_path)
...
def _execution_row(
    ...
    evidence_id = f"{item_type}_h1_{index:02d}"
    return {
        "execution_order": index,
        "execution_batch": "batch_01_h1_forecast_quality_source_review",
        "thesis_area": "H1",
        "source_id": source_id,
        ...
        "manual_execution_instruction_de": (
            "H1: Quelle manuell pruefen. Erst nach Page-/Section-Note, "

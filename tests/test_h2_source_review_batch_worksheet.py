...
from operations.project.build_h2_source_review_batch_worksheet import (
    WORKSHEET_COLUMNS,
    generate_h2_source_review_batch_worksheet,
)


def test_generate_h2_source_review_batch_worksheet_writes_five_rows(tmp_path: Path) -> None:
    _write_fixture(tmp_path)

    result = generate_h2_source_review_batch_worksheet(repo_root=tmp_path)

    worksheet = pd.read_csv(result.worksheet_path)
    doc = result.docs_path.read_text(encoding="utf-8")

    assert tuple(worksheet.columns) == WORKSHEET_COLUMNS
    assert result.worksheet_rows == 5
    assert result.unique_sources == 3
    assert result.method_rows == 3
    assert result.interpretation_rows == 2
    assert result.pending_citation_rows == 5
    assert result.final_release_ready_rows == 0
    assert worksheet["worksheet_order"].tolist() == list(range(1, 6))
    assert set(worksheet["thesis_area"]) == {"H2"}
    assert set(worksheet["selected_table"]) == {"T3"}
    assert set(worksheet["selected_figure"]) == {"F2"}
    ...
    assert "H2 Source Review Batch Worksheet" in doc
    assert "Worksheet rows: 5" in doc
    assert "Final release ready rows: 0" in doc
    assert chr(223) not in doc


def test_h2_source_review_batch_worksheet_keeps_causal_boundary_visible(
    tmp_path: Path,
) -> None:
    _write_fixture(tmp_path)
...
def test_h2_source_review_batch_worksheet_rejects_final_ready_execution_row(
    tmp_path: Path,
) -> None:
    _write_fixture(tmp_path)
    ...
    with pytest.raises(ValueError, match="0 final-ready execution rows"):
        generate_h2_source_review_batch_worksheet(repo_root=tmp_path)
...
def _execution_row(
    ...
    return {
        ...
        "execution_batch": "batch_02_h2_event_window_source_review",
        "thesis_area": "H2",
        "source_id": source_id,
        ...
        "manual_execution_instruction_de": (
            "H2: Quelle manuell pruefen. Erst nach Page-/Section-Note, "
        ...
        "do_not_claim_de": (
            ...
            "und keine thesis-facing Claims ohne manuelle Entscheidung."
        ),
    }

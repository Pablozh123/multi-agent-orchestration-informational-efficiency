...
from operations.project.build_h2_source_review_ledger_fill_guide import (
    GUIDE_COLUMNS,
    generate_h2_source_review_ledger_fill_guide,
)


def test_generate_h2_source_review_ledger_fill_guide_writes_five_rows(
    tmp_path: Path,
) -> None:
    _write_fixture(tmp_path)

    result = generate_h2_source_review_ledger_fill_guide(repo_root=tmp_path)

    guide = pd.read_csv(result.guide_path)
    doc = result.docs_path.read_text(encoding="utf-8")

    assert tuple(guide.columns) == GUIDE_COLUMNS
    assert result.guide_rows == 5
    assert result.matched_ledger_rows == 5
    ...
    assert guide["guide_order"].tolist() == list(range(1, 6))
    assert set(guide["thesis_area"]) == {"H2"}
    assert set(guide["selected_table"]) == {"T3"}
    assert set(guide["selected_figure"]) == {"F2"}
    assert set(guide["ledger_match_status"]) == {"matched_h2_worksheet_to_ledger"}
    assert guide["ready_for_manual_ledger_entry"].map(_as_bool).all()
    assert not guide["ready_for_final_release"].map(_as_bool).any()
    assert "H2 Source Review Ledger Fill Guide" in doc
    assert "Guide rows: 5" in doc
    assert "Matched ledger rows: 5" in doc
    assert "Final release ready rows: 0" in doc
    assert chr(223) not in doc


def test_h2_source_review_ledger_fill_guide_keeps_allowed_fields_visible(
    tmp_path: Path,
) -> None:
    _write_fixture(tmp_path)
    ...
    assert "keine intraday-ueberclaims" in boundary_text
...
def test_h2_source_review_ledger_fill_guide_rejects_missing_ledger_match(
    tmp_path: Path,
) -> None:
    _write_fixture(tmp_path)
    ...
    with pytest.raises(ValueError, match="must match by source_id and evidence_id"):
        generate_h2_source_review_ledger_fill_guide(repo_root=tmp_path)
...
def _worksheet_row(
    ...
    evidence_id = f"{item_type}_h2_{index:02d}"
    return {
        "worksheet_id": f"h2_source_review_{index:02d}_{source_id}__{evidence_id}",
        ...
        "blocked_wording_target_de": "Blocked-Wording fuer H2 pruefen.",
        "causal_claim_boundary_de": (
            "Kausalclaim-Grenze: keine Kausalitaet und keine sofortige "
            "Marktreaktion behaupten."
        ),
        ...
        "blocked_actions_de": (
            ...
            "Kausalclaims, keine Intraday-Ueberclaims, keine Runtime-Agenten, "
            "keine Rohartefakt-Dumps, max 50 rows und llm_audit_log."
        ),

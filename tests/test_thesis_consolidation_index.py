...
def test_generate_thesis_consolidation_index_writes_artifact_map(tmp_path: Path) -> None:
    ...
    assert result.index_rows == 66
    assert "Thesis Consolidation Index" in doc
    assert "Indexed artifacts: 66" in doc
    ...
    assert "THESIS_H2_SOURCE_REVIEW_LEDGER_FILL_GUIDE.md" in doc
    ...
    assert "THESIS_H3_SOURCE_REVIEW_LEDGER_FILL_GUIDE.md" in doc
...
def test_thesis_consolidation_index_keeps_deferred_boundaries(tmp_path: Path) -> None:
    ...
    assert "h2_source_review_ledger_fill_guide" in joined
    assert "5 h2 guide rows" in joined
    assert "5 matched ledger rows" in joined
    ...
    assert "h3_source_review_ledger_fill_guide" in joined
    assert "8 h3 guide rows" in joined
    assert "8 matched ledger rows" in joined
...
def _write_fixture(root: Path) -> None:
    paths = [
        ...
        "docs/project/THESIS_H2_SOURCE_REVIEW_LEDGER_FILL_GUIDE.md",
        "data/results/thesis_h2_source_review_ledger_fill_guide.csv",
        ...
        "docs/project/THESIS_H3_SOURCE_REVIEW_LEDGER_FILL_GUIDE.md",
        "data/results/thesis_h3_source_review_ledger_fill_guide.csv",
